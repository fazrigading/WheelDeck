...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-017 | Implement WebSocket-over-TCP listener for `state` and `button` messages. | | |
| TASK-018 | Implement pairing flow with `pair_request`/`pair_response` and session token persistence. | | |
| TASK-019 | Implement heartbeat every ~2s with neutralize after 2 missed beats. | | |
