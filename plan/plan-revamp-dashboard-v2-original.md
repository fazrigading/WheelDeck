driving page:
1. remove the whole Top bar (which has "Driving" text) in the driving page.
...
13. Wheel arc axis red bar must start from 0 (at the middle), so it grow/fill the left arc if rotated to left; and right arc if rotated to right.
...
below is the "Sequential preset" (which is using Shift Up and Down) that I made (as the developer):
```text
...
4. if we can do user-customizeable button layout, can we create a module and/or set of buttons? for example: 1row x 5columns for audio player module; 4x4 H-Shifter module, etc.
