---
goal: Fix post-revamp mobile issues found 2026-09-10 (routing, donate, settings wiring, UI, plus Xbox guide)
version: 1.0
...
Branch `fix/mobile-revamp-followup` off `main` (post-PR #27 c256364). 7 follow-up problems logged in `TODO.md:15` after the 5-phase M3 revamp (Menu, Connect, Driving, Settings, About, Donate). This plan fixes them without destabilizing the revamp, plus adds the requested Steam/Xbox guide page.

Scope is strictly the 7 items + guide; `feature/revamp-dashboard` stays separate until this lands.
...
- **REQ-007 Steam/Xbox guide**: New guide page under Settings — pressable Xbox 360 layout (A/B/X/Y, D-pad, LB/RB, LT/RT, Start/Back, LS/RS + triggers) that fires the same `DashboardInput`/`sendButtonEvent` path so the desktop's InputMapper visual can be verified. Reuse `DashboardInput` + `ConnectionRepository` wiring; visual only on mobile, no new protocol. Accessible from Settings → "Steam setup guide" or "Controller tester".
...
- **CON-002**: Minimum new deps. Prefer `SharedPreferences` for binding overrides, `url_launcher` already present, no new packages for Xbox guide (custom painter + Grid).
...
|------|-------------|-----------|------|
| TASK-001 | Add explicit CTA to `ConnectionScreen` when `status==connected`: show `FilledButton("Ready to Drive")` below `ConnectionStatusCard` (or make card tappable). On tap, `Navigator.of(context).maybePop()` if pushed from Menu then rely on `_Routing` to show Driving, or `Navigator.pushReplacement(DrivingView)` for immediate. Also ensure `_Routing` still auto-shows Driving when status becomes connected outside the pushed route (no regression). File: `mobile/lib/ui/features/connection/views/connection_screen.dart`. |  |  |
| TASK-002 | Fix disconnect destination: change `DrivingView._onDisconnect` + `DrivingViewModel.disconnect` to navigate to `ConnectionScreen` instead of `MenuScreen`. Option: after `await _viewModel.disconnect()`, if mounted, `Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (_) => const ConnectionScreen()), (r) => r.isFirst)` or pop to Menu then push Connect. Keep `_Routing` fallback to `MenuScreen` but override in Driving context. File: `mobile/lib/ui/features/driving/views/driving_view.dart`. |  |  |
| TASK-003 | Verify `main.dart:75` `_Routing` + `MenuScreen` push semantics: if user is on `Menu → Connection` and status becomes connected, ensure Driving appears without requiring Back. Could also make Menu's Connect button use `push` and on connected use `pop` + let `_Routing` handle Driving. Test both paths. |  |  |
...
| TASK-005 | Same hardening for `AboutScreen._openRepo` for consistency. |  |  |
...
| TASK-007 | Wire Controller Type to Driving visuals: expose `ControllerType` in `DrivingViewModel` (load via `ControllerType.load()` in `init` + `refreshControllerType()`). In `DrivingView._buildDrivingContent`, conditionally show/hide `PedalPanel` and `DashboardPanel` per `controllerType` (steeringOnly → wheel only, steeringDashboard → no pedals, etc.). Ensure `WheelView` stays. Files: `mobile/lib/ui/features/driving/view_models/driving_view_model.dart`, `mobile/lib/ui/features/driving/views/driving_view.dart`. |  |  |
| TASK-008 | Make Pedal Layout live: after Settings pop, refresh Driving. Add `DrivingView.didChangeDependencies` or handle `Navigator.push(...).then((_) => _viewModel.refreshPedalLayout())` from Menu/Driving when Settings returns. Already have `refreshPedalLayout()` but never called; wire it. Also handle `ControllerType` refresh similarly. File: `mobile/lib/ui/features/driving/views/driving_view.dart`. |  |  |
| TASK-009 | Fix Reset clipping in portrait: wrap `SettingsScreen` `ListView` bottom in `SafeArea` + extra bottom padding (`80` for FAB clearance style) or move Reset into `bottomNavigationBar` with `SafeArea`. Verify portrait 360dp and landscape. File: `mobile/lib/ui/features/settings/views/settings_screen.dart`. |  |  |

### Phase 4 — Xbox 360 guide (REQ-007)

| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-010 | Create `mobile/lib/ui/features/guide/views/xbox_guide_screen.dart` — M3 scaffold, `AppBar` "Steam setup guide". Layout: top row LB/RB, middle: D-pad left + Xbox diamond (Y top, X left, B right, A bottom) + Start/Back, bottom: LT/RT triggers + LS/RS sticks. Each button is `InkWell` that calls `viewModel.press(ControlId)` or directly `DashboardInput.activate` / `ConnectionRepository.sendButtonEvent` via `DrivingViewModel` or dedicated `GuideViewModel`. Show pressed feedback (M3 `primaryContainer` highlight) and send `ActionType.press`/`release` so desktop InputMapper lights up. No new protocol. |  |  |
| TASK-011 | Wire guide entry: add `ListTile` "Steam setup guide →" in `SettingsScreen` (or `MenuScreen`) navigating to `XboxGuideScreen`. Use `SettingsScreen(coordinator: coordinator)` wiring or create `GuideViewModel` with `ConnectionRepository`. |  |  |
| TASK-012 | Add minimal test `xbox_guide_screen_test.dart` that pumps screen and taps A/B and verifies callback. |  |  |
...
- **ALT-003** Make Xbox guide send raw `ButtonId` instead of `ControlId` — rejected; reuse `ControlId` → `InputMapper` mapping so desktop needs no change.
...
- **DEP-003** No new deps. Xbox guide uses only `material` + `provider`.

## 5. Files

- **FILE-001** `mobile/lib/ui/features/connection/views/connection_screen.dart` — add Ready-to-Drive CTA
- **FILE-002** `mobile/lib/ui/features/driving/views/driving_view.dart` — fix disconnect destination, add recalibrate refresh wiring, Xbox guide entry point
...
- **FILE-010** `mobile/lib/ui/features/settings/views/settings_screen.dart` — wire overrides, SafeArea for Reset, guide entry
- **FILE-011** `mobile/lib/ui/features/guide/views/xbox_guide_screen.dart` — new Xbox 360 tester
- **FILE-012** `mobile/lib/ui/features/guide/view_models/guide_view_model.dart` — optional thin VM
- **FILE-013** `mobile/test/ui/features/connection/connection_screen_test.dart` — add Ready-to-Drive CTA test
- **FILE-014** `mobile/test/ui/features/donate/donate_screen_test.dart` — new url_launcher mock test (optional)
- **FILE-015** `mobile/test/ui/features/settings/settings_screen_test.dart` — add binding edit + reset clipping test
- **FILE-016** `TODO.md` — mark new problems checklist as in-progress/done per phase
- **FILE-017** `plan/fix-mobile-revamp-followup-1.md` — this plan
...
- **TEST-007** `xbox_guide_screen_test` — tap A/B/Start triggers `sendButtonEvent` with correct `ControlId`.
...
- **MANUAL-005** Xbox guide: each button lights desktop InputMapper (visual or log) and haptic feedback.
...
- **RISK-003** Xbox guide reusing `ControlId` may not map 1:1 to Xbox `ButtonId` for all Steam-required buttons (e.g. LT/RT triggers). Mitigation: map those to closest `ControlId` or send as `ButtonId` via `sendMappingMode` extension, document fallback.
- **ASSUMPTION-001** `feature/revamp-mobile-ui` (PR #27 c256364) is on `main` — this branch rebases off that.
- **ASSUMPTION-002** Desktop `InputMapper` already handles both `SimulatedKeyPress` and `ControllerButton` for dashboard controls — Xbox guide just reuses that.

## 8. Related Specifications / Further Reading

- `TODO.md:15` — source of the 7 problems + guide request
